        ...
        renderArrowPrev={(onClickHandler, hasPrev, label) =>
          hasPrev && (
            <button
              ...
              onClick={onClickHandler}
              title={label}
              className="absolute left-4 top-1/2 z-30 -translate-y-1/2  hover:bg-black/70  p-4 rounded-full transition duration-300 shadow-lg"
            >
              <FaAngleLeft size={15} />
            </button>
          )
        ...
        renderArrowNext={(onClickHandler, hasNext, label) =>
          hasNext && (
            <button
              ...
              onClick={onClickHandler}
              title={label}
              className="absolute right-4 top-1/2 z-30 -translate-y-1/2 bg-black/50 hover:bg-black/70  p-3 rounded-full transition duration-300 shadow-lg"
            >
        ...
        {slides.map((slide, index) => (
          <div key={index} className="relative rounded-xl overflow-hidden">
            ...
            <img
              src={slide.img}
              alt={`Slide ${index + 1}`}
              className="object-cover w-full h-[70vh] md:h-[80vh]"
            />
            ...
            <div className="absolute inset-0 z-20 flex items-center px-6 lg:px-16">
              <motion.div
                className="text-white max-w-2xl backdrop-blur-[1px] p-4 rounded-lg"
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.9 }}
              >
                <motion.h2
                  className="text-3xl md:text-5xl font-extrabold mb-4 drop-shadow-2xl"
                  initial={{ opacity: 0, x: -40 }}
