...
const CourtBookingPage = () => {
  ...
  const { data: courts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await axios.get(`https://server12-taupe.vercel.app/courts`, {
        params: {
          page: currentPage,
          limit: pageSize,
          sort: sortOrder,
  ...
  return (
    <div data-aos="fade-right" className="max-11/12 mx-auto p-6 ">
      ...
      <Dialog open={isModalOpen} onClose={() => setIsModalOpen(false)} className="relative z-50">
        ...
        <div data-aos="zoom-in" className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="bg-white w-full max-w-md p-6 rounded-lg shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end space-x-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-2xl border bg-red-600 text-white">
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-4 py-2 rounded-2xl bg-green-600 text-white  hover:bg-green-700 disabled:opacity-50"
